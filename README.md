...
OAuth is better suited to developing interactive web applications that need to access a user's data from an external web service; but with a little IRB magic, it's actually pretty easy to authenticate a non-interactive script to do the same thing. Here's a quick and dirty tutorial on obtaining the OAuth keys this script requires:
